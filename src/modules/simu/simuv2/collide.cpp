/***************************************************************************
    ...
    email                : torcs@free.fr
    version              : $Id: collide.cpp 3948 2011-10-08 07:27:25Z wdbee $
...
static void SimCarCollideResponse(void * /*dummy*/, DtObjectRef obj1, DtObjectRef obj2, const DtCollData *collData)
{
	...
	for (i = 0; i < 2; i++) {
		...
		if ((car[i]->carElt->_state & RM_CAR_STATE_FINISH) == 0) {
			float dammage = (tdble) ((CAR_DAMMAGE * fabs(j) * damFactor * simDammageFactor[car[i]->carElt->_skillLevel]));
			dammage *= (tdble) MIN(1.5, dammage / 500.0);
			if (dammage < 10)
				dammage = 0;
		...
		sgMakeCoordMat4(carElt->pub.posMat, car[i]->DynGCg.pos.x, car[i]->DynGCg.pos.y,
						car[i]->DynGCg.pos.z - carElt->_statGC_z, (tdble) RAD2DEG(carElt->_yaw),
						(tdble) RAD2DEG(carElt->_roll), (tdble) RAD2DEG(carElt->_pitch));
...
static void SimCarWallCollideResponse(void *clientdata, DtObjectRef obj1, DtObjectRef obj2, const DtCollData *collData)
{
	...
	if (fabs(car->VelColl.az) > VELMAX) {
		car->VelColl.az = (tdble) (SIGN(car->VelColl.az) * VELMAX);
	}
	...
	sgMakeCoordMat4(carElt->pub.posMat, car->DynGCg.pos.x, car->DynGCg.pos.y,
					car->DynGCg.pos.z - carElt->_statGC_z, (tdble) RAD2DEG(carElt->_yaw),
					(tdble) RAD2DEG(carElt->_roll), (tdble) RAD2DEG(carElt->_pitch));
	dtSelectObject(car);
	dtLoadIdentity();
	dtTranslate(-carElt->_statGC_x, -carElt->_statGC_y, 0.0f);
